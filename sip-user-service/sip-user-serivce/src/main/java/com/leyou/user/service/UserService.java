...
public class UserService {
    ...
    public Boolean checkData(String data, Integer type) {
        ...
        switch (type) {
            ...
            case 2:
                ...
                break;
            default:
                throw new SipException(ExceptionEnum.INVALID_PARAM);
        }
    ...
    public void register(User user, String code) {
        ...
        if (!StringUtils.equals(code, value)) {
            //验证码不匹配
            throw new SipException(ExceptionEnum.VERIFY_CODE_NOT_MATCHING);
        }
    ...
    public User queryUser(String username, String password) {
        ...
        if (user == null) {
            throw new SipException(ExceptionEnum.USER_NOT_EXIST);
        }

        //检验密码是否正确
        if (!StringUtils.equals(CodecUtils.md5Hex(password, user.getSalt()), user.getPassword())) {
            //密码不正确
            throw new SipException(ExceptionEnum.PASSWORD_NOT_MATCHING);
        }
